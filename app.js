...
function renderNoteCard(){
  ...
  panel.innerHTML = `
    ...
    <div class="note-nav">
      ...
      <button class="btn btn-primary" id="next-card-btn">${ (notesState.cardIndex === mod.cards.length - 1 && notesState.moduleIndex === NOTES_MODULES.length - 1) ? 'Finish module' : 'Next' } &rarr;</button>
    </div>
  `;

  document.getElementById('prev-card-btn').onclick = prevCard;
  document.getElementById('next-card-btn').onclick = nextCard;
